...
class SiteParser extends EventEmitter {
  constructor (baseUrl, priceSelector, imgSelector) { // optional params
    super()
  ...
  fetchPrices (cb) {
    ...
    eachSeries(this.products, function (product, done) {
      ...
      self.x(product.link, { price: product.customSelector || self.priceSelector, image: self.imgSelector})(function (err, obj) {
        ...
        self.emit('priceFetched', product, obj.price, obj.image)

        if (!self._isPriceValid(product, obj.price)) {
          debug('priceAlert for', product, '\ngot new price:', obj.price)
          console.log(obj.image)
          self.emit('priceAlert', product, obj.price, obj.image)
        }
