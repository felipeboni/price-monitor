...
class PriceMonitoring extends EventEmitter {
  constructor(opts) {
    ...
    if (!opts.hasOwnProperty("parsersDir"))
      throw Error("Please provide a parsersDir property");
  ...
  onParserPriceAlert(product, newPrice, imageUrl) {
    this.emit("priceAlert", product, newPrice, imageUrl);
  }
  ...
  watchPrices(cb) {
    ...
    this.startWatch = function () {
      self.watchLoop = setTimeout(function () {
        self.fetchAllPrices(function () {
          /* all prices from all the parsers fetched */ self.round++;
  ...
  getProducts() {
    return _.flatten(
      this.parsers.map(function (pInstance) {
